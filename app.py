...
@st.cache_resource
def treinar_modelo_nativo(df_treino):
    # Separa as variáveis que o usuário vai mexer
    colunas_features = ['Placement', 'Advantage_Plus_Placements', 'Advantage_Plus_Creative', 
                        'Campaign_Objective', 'Targeting', 'Amount_spent', 'Frequency']
    
    # Prepara os dados limpos
    X = df_treino[colunas_features]
    y = df_treino['ROAS'].fillna(0) # Proteção contra dados vazios
    
    # Transforma texto em número (One-Hot Encoding)
    X_encoded = pd.get_dummies(X, drop_first=True)
    
    # Treina o modelo Random Forest na hora (leva milissegundos)
    modelo = RandomForestRegressor(n_estimators=100, random_state=42)
    modelo.fit(X_encoded, y)
    
    return modelo, list(X_encoded.columns)

df = carregar_dados()
# O Streamlit treina a IA 1 vez e guarda na memória
modelo_roas, colunas_treino = treinar_modelo_nativo(df)
...
with aba4:
    ...
    if df_filtrado.empty:
        ...
        if len(df_serie) > 0:
            ...
            # Correção: Separar a linha do texto para evitar o bug de soma de datas do Plotly
            fig_projecao.add_vline(x=ultima_data, line_dash="dash", line_color="red")
            fig_projecao.add_annotation(x=ultima_data, y=df_grafico['ROAS'].max(), text="Hoje", showarrow=False, yshift=15, font=dict(color="red"))
